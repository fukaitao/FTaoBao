...
public class WeitaoRefreshListView extends ListView implements AbsListView.OnScrollListener {
    ...
    public WeitaoRefreshListView(Context context, AttributeSet attrs) {
        super(context, attrs);
        Log.d("txxz", "WeitaoRefreshListView.WeitaoRefreshListView()");
        initView();
        this.setOnScrollListener(this);
    }

    private void initView() {
        Log.d("txxz", "WeitaoRefreshListView.initView()");
        initHeader();
        initFooter();
    }
    ...
    private void initHeader() {
        Log.d("txxz", "WeitaoRefreshListView.initHeader()");
    ...
    private void initFooter() {
        Log.d("txxz", "WeitaoRefreshListView.initFooter()");
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        switch (ev.getAction()) {
            case MotionEvent.ACTION_DOWN:
                Log.d("txxz", "WeitaoRefreshListView.onTouchEvent()--ACTION_DOWN");
                downY = (int) ev.getY();
                break;
            case MotionEvent.ACTION_MOVE:
                ...
                if (firstVisibleItemPosition == 0 && -headerViewHeight < paddingTop) {
                    if (paddingTop > 0 && currentState == DOWN_PULL_REFRESH) { // 完全显示了——松开刷新
                        Log.d("txxz", "WeitaoRefreshListView.onTouchEvent()--ACTION_MOVE-松开刷新");
                        currentState = RELEASE_REFRESH;
                        refreshHeaderView();
                    } else if (paddingTop < 0 && currentState == RELEASE_REFRESH) { // 没有显示完全——下拉刷新
                        Log.d("txxz", "WeitaoRefreshListView.onTouchEvent()--ACTION_MOVE-下拉刷新");
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (currentState == RELEASE_REFRESH) {
                    Log.d("txxz", "WeitaoRefreshListView.onTouchEvent()--ACTION_UP-刷新数据");
                    Log.i(TAG, "刷新数据");
                    // 把头布局设置为完全显示状态
                    ...
                    Log.d("txxz", "WeitaoRefreshListView.onTouchEvent()--ACTION_UP-放弃下拉刷新");
                    // 隐藏头布局
                    headerView.setPadding(0, -headerViewHeight, 0, 0);
                }
                break;
    ...
    private void refreshHeaderView() {
        Log.d("txxz", "WeitaoRefreshListView.refreshHeaderView()");
        switch (currentState) {
    ...
    public void onScrollStateChanged(AbsListView view, int scrollState) {
        Log.d("txxz", "WeitaoRefreshListView.onScrollStateChanged()");
    ...
    public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
        Log.d("txxz", "WeitaoRefreshListView.onScroll()");
